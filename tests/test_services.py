"""Tests del pipeline POST /escaneo y servicios internos."""
...
def test_post_escaneo_pipeline(monkeypatch, tmp_path, example_geometry):
    from app.core import paths
    from app.main import app
    from app.routers import escaneo as escaneo_route
    ...
    monkeypatch.setattr(
        escaneo_route,
        "upload_bytes_to_r2",
        lambda content, object_key: object_key,
    )
    ...
    with ply_path.open("rb") as handle:
        resp = client.post(
            "/escaneo",
            files={"archivo": ("munon.ply", handle, "application/octet-stream")},
    ...
    body = resp.json()
    assert body["job_id"]
    assert body["object_key"].startswith("escaneos/")
    assert body["download_urls"]["stl"] == f"/escaneo/{body['job_id']}/stl"
