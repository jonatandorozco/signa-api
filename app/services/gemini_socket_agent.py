"""
Agente de diseño de socket con Gemini (prompt en app/prompts/socket_design_system.md).
"""
...
from app.services.llm_client import chat_json_completion, get_llm_client, get_model_name
...
def run_gemini_socket_agent(
    ...
    *,
    ...
    prefix = f"[job={job_id}]" if job_id else "[job=unknown]"
    ...
    logger.info(
        "%s gemini request start model=%s prompt_chars=%d user_chars=%d",
        prefix,
        model_name,
        len(system_prompt),
        len(user_message),
    ...
    logger.info(
        "%s gemini response done (%.1fs) response_chars=%d",
        prefix,
    ...
    try:
        ...
        parsed["design_parameters"]["agent_engine"] = "gemini"
        parsed["design_parameters"]["llm_model"] = model_name
        parsed["design_parameters"]["llm_provider"] = "gemini"
        ...
        logger.warning("%s gemini parse failed, falling back to rules: %s", prefix, exc)
        ...
        rules["design_parameters"]["gemini_error"] = str(exc)
        rules["design_parameters"]["llm_model"] = model_name
        rules["design_parameters"]["llm_provider"] = "gemini"
        msgs = list(rules.get("quality_gate", {}).get("messages", []))
        msgs.append(f"Fallback a reglas: error Gemini ({exc})")
